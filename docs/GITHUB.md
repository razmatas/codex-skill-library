...
Public repository: [warung-kerja/codex-skill-library](https://github.com/warung-kerja/codex-skill-library), moved from `razmatas/codex-skill-library` and verified on 2026-09-22. Raz explicitly wants agents to clone it without account-specific repository access. All 45 audited personal copies are archived with exact manifest hashes and original roots/entry filenames. Included licenses remain intact; unknown licenses/authorship are recorded rather than inferred. Public visibility does not establish redistribution permission for entries whose licenses remain unknown. No automatic installation is enabled. Explicit restore instructions: RESTORE.md.
