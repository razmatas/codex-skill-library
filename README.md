...
Public source repository: [warung-kerja/codex-skill-library](https://github.com/warung-kerja/codex-skill-library). Raz explicitly chose public visibility so other agents can clone it without account-specific repository access. See [docs/GITHUB.md](docs/GITHUB.md). All 45 audited personal package copies are archived under `skills/<original-root>/<folder>` with exact fingerprints and included licenses preserved. Forty-one are discoverable; four retain nonstandard entry files as backup-only. Public visibility does not resolve unknown authorship or grant redistribution rights; entries without verified licenses remain clearly marked for review. Built-in/plugin caches and credentials are excluded. Cloning does not install skills; use [docs/RESTORE.md](docs/RESTORE.md).

Current version includes live read-only inventory, reporting/service helpers, an audited snapshot backup and an explicit dry-run/apply restore command. Scanning/reporting never automatically installs, publishes or reconciles packages. Raz and Gabs have been paired; the dashboard distinguishes live from stale reports. See [docs/REVIEW.md](docs/REVIEW.md) for the latest comparison and preserved drift.
...
The fixture-tested apply module is exposed through the explicit restore command. It requires an exact plan digest, copies a reviewed plain package to a hidden stage, checks the staged hash and skill name, rechecks the target, and retains updated packages in the install root's hidden `.skill-library-backups/`. It does not delete backups or failed stages. A receipt records ownership only after installation; unchanged unmanaged packages are not automatically adopted. Runs use an advisory `.sync-lock`; stop other installers during apply. Failures may leave recoverable staging or backups and previously completed entries; this is not a multi-package atomic transaction. Interrupted runs require explicit recovery. Upstream matching, recovery CLI and two-machine verification remain incomplete.
