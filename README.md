...
Private source repository: [razmatas/codex-skill-library](https://github.com/razmatas/codex-skill-library). See [docs/GITHUB.md](docs/GITHUB.md). All 44 audited personal package copies are archived under `skills/<original-root>/<folder>` with exact fingerprints and included licenses preserved. Forty are discoverable; four retain nonstandard entry files as backup-only. Unknown licenses/authorship remain explicitly unknown: this private backup does not grant public redistribution permission. Built-in/plugin caches and credentials are excluded. Cloning does not install skills; use [docs/RESTORE.md](docs/RESTORE.md).

Current version includes live read-only inventory, reporting/service helpers, private snapshot backup and an explicit dry-run/apply restore command. Scanning/reporting never automatically installs, publishes or reconciles packages. Two-machine setup remains unverified until Gabs is paired and restored deliberately.
