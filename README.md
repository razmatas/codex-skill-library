...
Public source repository: [razmatas/codex-skill-library](https://github.com/razmatas/codex-skill-library). Raz explicitly chose public visibility so other agents can clone it without account-specific repository access. See [docs/GITHUB.md](docs/GITHUB.md). All 44 audited personal package copies are archived under `skills/<original-root>/<folder>` with exact fingerprints and included licenses preserved. Forty are discoverable; four retain nonstandard entry files as backup-only. Public visibility does not resolve unknown authorship or grant redistribution rights; entries without verified licenses remain clearly marked for review. Built-in/plugin caches and credentials are excluded. Cloning does not install skills; use [docs/RESTORE.md](docs/RESTORE.md).

Current version includes live read-only inventory, reporting/service helpers, an audited snapshot backup and an explicit dry-run/apply restore command. Scanning/reporting never automatically installs, publishes or reconciles packages. Raz and Gabs are paired and reporting fresh inventories; see [docs/REVIEW.md](docs/REVIEW.md) for the latest comparison and preserved drift.
